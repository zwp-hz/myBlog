...
let imgs: string[] = [
  ...
  img_styles: any = [], // 图片样式参数
  img_width: number = 0, // 图片的宽度
  row_number: number = 0, // 图片行数
  row_tops: any = [], // 记录最好一行图片的位置
  load_type: string = 'init', // 加载类型 默认：init - 初始化  scroll - 滚动加载  resize - 视图变化
  beforeScrollTop: number = 0, // 记录滚动条变化前的数值 用于区分上下滚动
  timer: any
...
export default class fallsFlow extends Vue {
  @State('IMGHOST')
  IMGHOST: string
  @State('QN_POSTFIX')
  QN_POSTFIX: string
  ...
  embed_nav: { list: string[]; index: number } = {
    list: ['js', 'html', 'css'],
    index: 0
  }
  ...
  mounted() {
    const codes: any = document.querySelectorAll('pre code')
   ...
   */
  setLineNumber(index: number = 0): void {
    this.embed_nav.index = index

    this.$nextTick(() => {
      const code: any = this.$refs[`code${index}`]

      this.line_number = Math.ceil(code.offsetHeight - 40) / 20
    })
  }
   ...
   * @param {Array} img_list - 图片列表
   */
  imgRrgodic(img_list: any[]): void {
    const content: any = this.$refs.content,
      offsetWidth: number = content.offsetWidth, // 容器的宽度
      img_dom_length: number = img_styles.length
   ...
   * @param {Number} index - 图片下标
   */
  setImgPosition(index: number = 0): void {
    const content: any = this.$refs.content
    let number =
      load_type === 'resize' ? 0 : content.getElementsByClassName('box').length
      ...
      for (let i = number; img_styles.length > i; i++) {
        ...
        if (item) {
          ...
          if (row_tops.length < row_number) {
            ...
            // 得到最小值
            let min: number = Math.min(...row_tops)
   ...
   * @param {Function} callback - 回调函数
   */
  imgLoad(src: string, callback: any): void {
    let img = new Image()
    img.src = src
